    ...
    <div className="space-y-4">
      {results.map((r) => (
        <div key={r.id} className="group cursor-pointer">
          <li className="relative flex flex-col gap-4 rounded-2xl border border-zinc-900 bg-zinc-950/30 hover:bg-zinc-900/40 p-5 transition-all list-none">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0">
                <div className="p-1.5 rounded-lg bg-indigo-500/10 text-indigo-400 border border-indigo-500/10 shrink-0">
                  <FileText className="w-4 h-4" />
                </div>
                <span className="text-xs font-bold text-zinc-300 group-hover:text-indigo-400 transition-colors uppercase tracking-tight truncate">{r.source}</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-lg bg-white/[0.03] border border-white/5">
                  <Target className="w-3 h-3 text-emerald-500" />
                  <span className="text-[10px] font-mono font-bold text-emerald-500">{(r.score * 100).toFixed(2)}% MATCH</span>
                </div>
            ...
            <p className="text-sm text-zinc-400 leading-relaxed font-medium line-clamp-2 italic border-l-2 border-indigo-500/20 pl-4 py-1">
              "{r.snippet}"
            </p>
            ...
            <div className="flex items-center justify-between mt-1">
              <div className="flex gap-2">
                {r.tags.map(tag => (
                  <Badge key={tag} className="bg-zinc-900 text-zinc-500 border-zinc-800 text-[9px] font-bold px-1.5 py-0">{tag}</Badge>
                ))}
              </div>
              <ChevronRight className="w-4 h-4 text-zinc-700 group-hover:text-indigo-400 group-hover:translate-x-1 transition-all" />
            </div>
          </li>
        </div>
      ))}
